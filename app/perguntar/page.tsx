...
export default function App() {
  ...
  const isFormDisabled = isLoading || answered;
           ...
           {" "}
            ...
            {" "}
              ...
              <p className="text-gray-600 text-base text-center max-w-md">
                Pergunte sobre inovação, startups, tecnologia ou
                ODS 9 no Brasil. {" "}
              </p>
              {" "}
            </header>
            {!answered || isLoading ? (
              ...
              >
                <textarea
                  ...
                  className="rounded-2xl p-5 text-lg border border-gray-200 shadow-inner resize-none focus:outline-none focus:ring-4 focus:ring-[#8ecae6]/70 transition-all duration-300 text-gray-900 bg-white/70 backdrop-blur-md hover:bg-white/90 placeholder:text-gray-400 disabled:bg-gray-100 disabled:cursor-not-allowed"
                  aria-label="Pergunta"
                />

                <button
                  type="submit"
                  disabled={isFormDisabled || !question.trim()}
                  aria-busy={isLoading}
                  aria-disabled={isFormDisabled || !question.trim()}
                  className={`
              ...
              <div className="mt-8">
                <h3 className="text-lg font-semibold text-[#023047] mb-2">
                  Sua Pergunta:
                </h3>
                <p className="rounded-2xl p-5 text-lg border border-gray-200 shadow-inner resize-none bg-gray-100 text-gray-700 break-words">
                  {question}
                </p>
              </div>
            )}
          ...
          {answered && !isLoading && answer.trim() && (
            ...
            >
              ...
              <div className="flex justify-end pt-4 border-t border-gray-200">
                ...
                <button
                  onClick={handleReset}
                  className="flex items-center space-x-2 px-6 py-2 rounded-xl font-semibold text-[#023047] border border-gray-300 bg-gray-100 shadow-sm transition-all duration-300 hover:bg-white hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[#8ecae6]"
                >
                  {" "}
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
